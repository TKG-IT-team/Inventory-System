...
if __name__ == "__main__":
    ...
    if len(since_date) == 0:
        ...
        shopify_full_order_df = ShopifyOrderAPI.generate_full_order_df()
        #For Shopee
        print("Getting orders from Shopee...")
        shopee_full_order_df = ShopeeAPI.generate_full_order_df()
        #For Lazada
        print("Getting orders from Lazada...")
        lazada_full_order_df = LazadaOrderAPI.generate_full_order_df()
    else:
        #For Shopify
        print(f"Getting orders from Shopify since {since_date}...")
        shopify_full_order_df = ShopifyOrderAPI.generate_new_order_df(since_date)
        #For Shopee
        print(f"Getting orders from Shopee since {since_date}...")
        shopee_full_order_df = ShopeeAPI.generate_new_order_df(since_date)
        #For Lazada
